...
import {
  registroProcurador,
  gerarCodigoProcurador,
  validarCodigoProcurador,
  requestError,
  redefinirSenhaProcurador
} from '../services/procuradorApi.ts'
import { cpfCnpjValido } from '../utils/formatCpfCnpj'

export const STEP_META = [{ label: "CPF" }, { label: "Código" }, { label: "Senha" }];
...
export const stepSubtitles: Record<number, string> = {
  1: "Digite o CPF cadastrado como procurador em algum processo.",
  2: "Digite o código enviado para o e-mail cadastrado.",
  3: "Defina uma senha segura para acessar o sistema.",
};

type PropositoProcurador = 'registro_procurador' | 'redefinicao_senha_procurador';

export function useProcurador(proposito: PropositoProcurador, errorMessager: string) {
  ...
  const handleGerarCodigo = async () => {
    ...
    setLoading(true);
    try {
      await gerarCodigoProcurador(cnpj, proposito);
      setStep(2);
    } catch (err) {
  ...
  const handleValidarCodigo = async () => {
    ...
    setLoading(true);
    try {
      await validarCodigoProcurador(cnpj, codigo, proposito);
      setStep(3);
    } catch (err) {
  ...
  const handleRegistrar = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await registroProcurador(cnpj, password);
  ...
  const handleRedefinirSenha = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await redefinirSenhaProcurador(cnpj, password);
      navigate("/procurador/login");
    } catch (err) {
